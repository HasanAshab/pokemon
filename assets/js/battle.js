...
function showMovesImpactByField(powerReducedMoveTypes,criticalMoveTypes,playerTag){
  const cardWrappers = document.querySelectorAll(`.${playerTag}-controle-cont .card-container .single-card-wrapper`)
  cardWrappers.forEach(cardWrapper => {
    const card = cardWrapper.querySelector(".card")
    const moveId = card.dataset.moveId
    const move = new Move(moveId)
    if (powerReducedMoveTypes.includes(move.type))
      card.classList.add("power-reduced")
    else
      card.classList.remove("power-reduced")

    if (criticalMoveTypes.includes(move.type))
...
function displayBattleFieldImpacts() {
  ...
  showMovesImpactByField(powerReducedMoveTypes,criticalMoveTypes,"you")
  showMovesImpactByField(powerReducedMoveTypes,criticalMoveTypes,"enemy")
