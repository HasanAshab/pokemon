...
globalThis.duplicatePokemon = function duplicatePokemon(name) {
  const pokemonsMeta = JSON.parse(localStorage.getItem("pokemons-meta")) || {};
  const originalMeta = pokemonsMeta[name];
  ...
  let duplicateName = name + "_copy";
  let counter = 1;
  
  // Ensure unique name
  while (pokemonsMeta[duplicateName]) {
    duplicateName = name + "_copy" + counter;
    counter++;
  }
  
  // Create a deep copy of the original Pokemon
  const duplicateMeta = JSON.parse(JSON.stringify(originalMeta));
  
  // Reset some stats for the duplicate
  duplicateMeta["wins-count"] = 0;
  duplicateMeta["loses-count"] = 0;
  duplicateMeta.isSelectedForMultiBattle = false;
  
  // Restore full health
  if (duplicateMeta.stats && duplicateMeta.stats.hp !== undefined) {
    const pokemon = new Pokemon(duplicateMeta.id, duplicateMeta);
    duplicateMeta.stats.hp = pokemon.maxhp;
  }
  
  // Add the duplicate to the collection
  pokemonsMeta[duplicateName] = duplicateMeta;
  localStorage.setItem("pokemons-meta", JSON.stringify(pokemonsMeta));
  
  // Reload the Pokemon list
  loadAllPokemons();
  
  alert(`Pokemon duplicated as "${duplicateName}"!`);
};
